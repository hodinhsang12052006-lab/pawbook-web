...
interface ProfileContentProps {
  ...
  initialWalletHistory: any[];
  initialBookings: { received: any[]; sent: any[] };
}
...
export default function ProfileContent({
  ...
  const [currentUser, setCurrentUser] = useState<any>(initialSessionUser);
  const [profile, setProfile] = useState<any | null>(initialProfile);
  const [loading, setLoading] = useState(false);
  const [isOwnProfile] = useState(!resolvedUid || (initialSessionUser && initialSessionUser.id === resolvedUid));
  ...
  const [walletHistory, setWalletHistory] = useState<any[]>(initialWalletHistory);
  const [activeRightTab, setActiveRightTab] = useState<"bookings" | "posts">("posts");
  const [bookings, setBookings] = useState<{ received: any[]; sent: any[] }>(initialBookings);
  ...
  // User's own posts
  const [myPosts, setMyPosts] = useState<PostType[]>(initialPosts);
      ...
      <main className="mx-auto flex-1 w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-6">
          ...
          <div className="hidden lg:block lg:col-span-5 space-y-6">
            {/* CV Manager Component (Only for own profile) or Candidate CV download link */}
            {isOwnProfile ? (
              <CVManager />
            ) : (
              profile.cv_url && (
                          ...
                          {req.status === "PENDING" && (
                            <div className="flex items-center gap-2 justify-end">
                              ...
                              <button
                                ...
                                className="px-3 py-1.5 rounded-lg border border-emerald-500/25 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-450 text-[10px] font-bold transition-all cursor-pointer"
                              >
                                ✅ Nhận đơn
                              </button>
                            </div>
                    ...
                    <div className="space-y-4 divide-y divide-slate-850/50">
                      {bookings.sent.map((req: any, idx: number) => (
                        <div key={req.id} className={`${idx > 0 ? "pt-4" : ""} space-y-3 text-xs`}>
                          ...
                          <div className="bg-slate-950/40 border border-slate-850 p-3 rounded-xl space-y-2">
                            <p className="text-3xs font-semibold text-slate-555 uppercase tracking-wider">
                              Đối với bài đăng:{" "}
                              <span className="text-blue-400">
                                {req.job?.title || req.service?.name}
                              </span>
              ...
              <div className="space-y-6">
                ...
                {loadingPosts ? (
                  <div className="flex flex-col items-center justify-center py-12 gap-2 bg-slate-900/10 border border-slate-850 rounded-2xl">
                    ...
                    <span className="text-xs text-slate-455">Đang tải bài viết...</span>
                  </div>
                ) : myPosts.length === 0 ? (
                  <div className="rounded-2xl border border-slate-800 bg-slate-900/10 p-12 text-center flex flex-col items-center justify-center space-y-2">
                    ...
                    <p className="text-xs text-slate-455 italic">Thành viên này chưa có bài viết nào.</p>
                  </div>
                ) : (
