...
export default function TechnicianRegisterForm() {
  ...
  const handleFinishStep1 = async () => {
    ...
    try {
      ...
      setLoading(false);
      if (!signInResult?.ok) {
        setError("Tạo tài khoản thành công nhưng đăng nhập thất bại. Vui lòng đăng nhập lại.");
        return;
      }
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        setError(data.error || "Không thể lưu hồ sơ. Vui lòng thử lại.");
        setLoading(false);
        return;
      }
      refreshSession();
    ...
    <div className="mx-auto w-full max-w-md space-y-6">
      <div className="space-y-2 text-center lg:text-left">
        <h2 className={`text-2xl font-extrabold ${isDark ? "text-white" : "text-slate-900"}`}>
          {step === 1 && "💅 Tìm việc nail nhanh"}
          {step === 2 && "Tay nghề & mong muốn"}
          {step === 3 && "Ảnh mẫu móng của bạn"}
        </h2>
        <p className={isDark ? "text-slate-400 text-sm" : "text-slate-500 text-sm"}>
          Bước {step}/3 — Đăng ký chưa tới 60 giây
        </p>
      </div>
      ...
      {step === 1 && (
        <div className="space-y-4 animate-fadeIn">
          <div>
            <label className={`block text-sm font-bold mb-1.5 ${labelClass}`}>Tên / Nickname</label>
            <div className="relative">
              <UserIcon className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-500" />
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="VD: Kim Nails" className={`${inputClass} pl-11`} />
            </div>
          </div>

          <div>
            <label className={`block text-sm font-bold mb-1.5 ${labelClass}`}>Số điện thoại</label>
          ...
          <button
            ...
            disabled={loading}
            className={`w-full flex items-center justify-center gap-2 rounded-xl bg-pink-600 hover:bg-pink-500 py-3.5 text-base font-bold text-white shadow-lg shadow-pink-600/25 disabled:opacity-50 ${PRESS}`}
          >
            {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><span>Tiếp tục</span><ArrowRight className="h-5 w-5" /></>}
          </button>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="space-y-5 animate-fadeIn">
          <div>
            <label className={`block text-sm font-bold mb-2 ${labelClass}`}>Tay nghề chính (chạm để chọn)</label>
            <div className="flex flex-wrap gap-2">
              {SKILL_OPTIONS.map((s) => (
                <button
                  ...
                  className={`rounded-full px-3.5 py-2.5 text-sm font-bold border-2 transition-colors ${PRESS} ${specialties.includes(s) ? "border-pink-500 bg-pink-500/15 text-pink-400" : `${cardClass} ${labelClass}`}`}
                >
                  {s}
                </button>
              ))}
          ...
          <div>
            <label className={`block text-sm font-bold mb-2 ${labelClass}`}>Hình thức lương mong muốn</label>
            <div className="grid grid-cols-2 gap-2 mb-2">
              <button
                type="button"
                onClick={() => setSalaryMode("Bao lương")}
                className={`rounded-xl py-2.5 text-sm font-bold border-2 ${PRESS} ${salaryMode === "Bao lương" ? "border-pink-500 bg-pink-500/10" : cardClass} ${isDark ? "text-white" : "text-slate-900"}`}
              >
                Bao lương
              </button>
              <button
                ...
                className={`rounded-xl py-2.5 text-sm font-bold border-2 ${PRESS} ${salaryMode === "Ăn chia %" ? "border-pink-500 bg-pink-500/10" : cardClass} ${isDark ? "text-white" : "text-slate-900"}`}
              >
                Ăn chia %
              </button>
            </div>
          ...
          <div>
            <label className={`block text-sm font-bold mb-2 ${labelClass}`}>Quyền lợi mong muốn</label>
            <div className="flex flex-wrap gap-2">
              {BENEFIT_OPTIONS.map((b) => (
                <button
                  ...
                  className={`rounded-full px-3.5 py-2.5 text-sm font-bold border-2 transition-colors ${PRESS} ${benefits.includes(b) ? "border-emerald-500 bg-emerald-500/15 text-emerald-400" : `${cardClass} ${labelClass}`}`}
                >
                  {b}
                </button>
              ))}
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => setStep(1)} className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3.5 font-bold ${cardClass} ${labelClass} ${PRESS}`}>
              <ArrowLeft className="h-5 w-5" />
            </button>
            <button
              type="button"
              onClick={() => setStep(3)}
              className={`flex-1 flex items-center justify-center gap-2 rounded-xl bg-pink-600 hover:bg-pink-500 py-3.5 text-base font-bold text-white shadow-lg shadow-pink-600/25 ${PRESS}`}
            >
              <span>Tiếp tục</span>
      ...
      {step === 3 && (
        <div className="space-y-5 animate-fadeIn">
          <div>
            <label className={`block text-sm font-bold mb-2 ${labelClass}`}>Tải 3-5 ảnh/video mẫu móng đẹp nhất</label>
            <div className="grid grid-cols-3 gap-2">
              ...
              {portfolioImages.length < 5 && (
                <button
                  ...
                  disabled={uploading}
                  className={`aspect-square rounded-xl border-2 border-dashed border-slate-700 flex flex-col items-center justify-center gap-1 text-slate-500 hover:border-pink-500 hover:text-pink-400 transition-colors ${PRESS}`}
                >
          ...
          <div>
            <label className={`block text-sm font-bold mb-2 ${labelClass}`}>Trạng thái hiện tại</label>
            <div className="space-y-2">
              <button
                type="button"
                onClick={() => setStatus("URGENT")}
                className={`w-full flex items-center gap-3 rounded-xl p-4 border-2 ${PRESS} ${status === "URGENT" ? "border-red-500 bg-red-500/10" : cardClass}`}
              >
                <Flame className={`h-5 w-5 ${status === "URGENT" ? "text-red-400" : "text-slate-500"}`} />
                <span className={`font-bold ${status === "URGENT" ? "text-red-300" : labelClass}`}>Đang rảnh tay / Cần việc ngay</span>
              </button>
              <button
                type="button"
                onClick={() => setStatus("BETTER")}
                className={`w-full flex items-center gap-3 rounded-xl p-4 border-2 ${PRESS} ${status === "BETTER" ? "border-blue-500 bg-blue-500/10" : cardClass}`}
              >
                <RefreshCw className={`h-5 w-5 ${status === "BETTER" ? "text-blue-400" : "text-slate-500"}`} />
                <span className={`font-bold ${status === "BETTER" ? "text-blue-300" : labelClass}`}>Tìm chỗ tốt hơn</span>
              </button>
            </div>
          </div>

          <div className="flex gap-3">
            <button type="button" onClick={() => setStep(2)} className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3.5 font-bold ${cardClass} ${labelClass} ${PRESS}`}>
              <ArrowLeft className="h-5 w-5" />
            </button>
            <button
              type="button"
              ...
              disabled={loading}
              className={`flex-1 flex items-center justify-center gap-2 rounded-xl bg-pink-600 hover:bg-pink-500 py-3.5 text-base font-bold text-white shadow-lg shadow-pink-600/25 disabled:opacity-50 ${PRESS}`}
            >
              {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><CheckCircle2 className="h-5 w-5" /><span>Hoàn Tất 🎉</span></>}
            </button>
          </div>
        </div>
      )}
