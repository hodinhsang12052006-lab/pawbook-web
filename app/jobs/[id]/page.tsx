...
export default function JobDetailPage({ params }: PageProps) {
  ...
  const handleSendBooking = async () => {
    if (!jobId || bookingSending) return;
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        toast.success("Nộp đơn ứng tuyển thành công! Nhà tuyển dụng đã được thông báo. 🎉");
        setIsBookingModalOpen(false);
        setBookingMessage("");
      } else {
        toast.error(data.error || "Gửi đơn ứng tuyển thất bại.");
      }
    } catch (err) {
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-slate-800 bg-slate-900/30 p-6 backdrop-blur-md space-y-5 sticky top-24">
              ...
              <button
                onClick={handleApply}
                disabled={applying}
                className="w-full flex items-center justify-center gap-2 rounded-xl bg-slate-800 border border-slate-700/60 py-2.5 text-xs font-semibold text-slate-200 hover:bg-slate-700 hover:text-white disabled:opacity-50 transition-all duration-200"
              >
                {applying ? (
                  <>
                  ...
                  <>
                    ...
                    <span>Nộp nhanh CV mặc định</span>
                  </>
                )}
              </button>
      ...
      {isBookingModalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 space-y-4 animate-scaleUp text-xs">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-4xs font-bold text-slate-400 mb-1">LỜI NHẮN / GIỚI THIỆU BẢN THÂN</label>
                <textarea
                  rows={4}
                  placeholder="Nhập lời giới thiệu ngắn của bạn hoặc mô tả kinh nghiệm liên quan..."
            ...
            <div className="flex items-center justify-end gap-2 border-t border-slate-800 pt-3">
              ...
              >
                {bookingSending ? "Đang gửi..." : "Gửi ứng tuyển"}
              </button>
            </div>
          </div>
        </div>
